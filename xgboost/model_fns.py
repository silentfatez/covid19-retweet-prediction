...
class EarlyStopping:
    ...
    def __init__(self, patience: int = 10, delta: int = 0, logger=None) -> None:
...
def train(
    ...
    if load_checkpoints:
        ...
        if not save_prefix:
            raise ValueError(
                "if load_checkpoints are not identified, please include save_prefix"
            )
    ...
    for epoch_no in range(1, n_epochs + 1):
        epoch_train_ls = random.choices(
            train_set, k=minibatch_unit * train_val_ratio[0]
        )
        ...
        overall_msle = _val_epoch(
            epoch_val_ls, model, relevant_features, main_proj_dir=main_proj_dir
        )
...
def _extract_x_y_featurenames(df: pd.DataFrame, relevant_features: list):
