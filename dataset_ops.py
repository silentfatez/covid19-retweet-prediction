...
def _get_train_val_test_size(dataset_len: int, split: tuple):
...
def _random_sample(row_ids: list, sample: int = None):
    ...
    if sample:
        if sample >= len(row_ids):
            raise ValueError(
                f"size of sample ({sample}) must be less than total no of rows ({len(row_ids)})"
            )
        row_ids = row_ids[:sample]
    return row_ids


def _train_val_test_split(row_ids: list, split: tuple = (0.8, 0.1, 0.1)):
